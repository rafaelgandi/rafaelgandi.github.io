...
let fileMaps = require('./filemap.json');
...
gulp.task('compile-component-sass', function() { // Compiler for sass files for components 
    return (
        gulp.src('../public/js/vectto/**/*.scss')
            .pipe(sourcemaps.init())
            .pipe(sass({
                outputStyle: 'compressed',
                includePaths: ['../public/js']
            }))
            .pipe(sourcemaps.write('.'))
            .on('error', sass.logError)
...
gulp.task('build', () => {
    ...
    console.log('\x1b[32m%s\x1b[0m', '================================================');
    let buildsArr = [];
    fileMaps = Builder.fileMapAdapter(fileMaps);
    ...
    // See: https://www.joezimjs.com/javascript/complete-guide-upgrading-gulp-4/
    gulp.watch('../public/js/vectto/**/*.js').on('all', (_event, _path) => {
    ...
    // SASS compiler 
    gulp.watch('../public/scss/**/*.scss', gulp.series('compile-sass')).on('change', (sassFile) => {
        console.log('SCSS compiled file ' + sassFile);
    });
    gulp.watch('../public/js/vectto/**/*.scss', gulp.series('compile-component-sass')).on('change', (sassFile) => {
