module.exports = function (_code) {
    ...
    _code.replace(/class\s+([a-zA-Z0-9_$]+)\s+extends\s+cm.ComponentElement/g, (match, className) => {
        _code = `${ _code } ; ${(replaceWithHtmlTag === '') ? `return ${ className };` : `return cm.createComponent(${ className }, ${ replaceWithHtmlTag });`}`;
    });      
    _code = `AMD.define((require, __import) => { "use strict"; const component = require('vectto/util/ComponentModule/cm'); \n${ _code }\n});`;
    //console.log(_code);
    return _code;
};